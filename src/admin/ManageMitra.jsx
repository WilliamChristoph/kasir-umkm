...
const ManageMitra = ({ session }) => {
  ...
  const filteredUsers = users.filter(user => {
    const matchesSearch =
      (search.namaToko
        ? user.storeName.toLowerCase().includes(search.namaToko.toLowerCase())
        : true) &&
      (search.namaPemilik
        ? user.ownerName.toLowerCase().includes(search.namaPemilik.toLowerCase())
        : true) &&
      ...
      if (u.id === userId) {
        return {
          ...
          bannedUntil: new Date(
            Date.now() + selectedDurasi * 24 * 60 * 60 * 1000
          ).toISOString()
        }
      }
      return u
        ...
        {filteredUsers.map(user => (
          <div key={user.id} className="rounded-xl bg-card p-4 shadow-sm">
            ...
            <p className="text-xs text-muted-italic">
              Daftar: {user.tanggalDaftar || new Date().toLocaleDateString()}
            </p>
